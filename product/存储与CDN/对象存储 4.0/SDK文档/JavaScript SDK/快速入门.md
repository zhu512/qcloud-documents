...
- 格式二：细粒度控制权限，后端通过获取临时密钥给到前端，只有在相同请求时，前端才重复使用临时密钥，后端可以通过 Scope 细粒度控制权限。
...
- 格式三：前端每次请求前都需要通过 getAuthorization 获取签名，后端使用固定密钥或临时密钥计算签名返回至前端。该格式分块上传权限不便控制，不推荐您使用此格式。
...
- 格式四：前端使用固定密钥计算签名，仅限开发调试用途，禁止在生产环境使用，会有密钥泄漏风险。
